...
export default function AdminPage() {
  ...
  async function testWinnerBroadcast() {
    const testWinner = { name: 'Test Winner', score: 1000 };
    const testTeams = [
      { id: 1, name: 'Team A', score: 500 },
      { id: 2, name: 'Team B', score: 1000 }
    ];
    ...
    try {
      ...
      showNotification('Test winner broadcast sent', 'success');
    } catch (e: any) {
  showNotification('Broadcast failed: ' + e.message, 'error');
}
  }

  // --- Test winner state function ---
  async function testWinnerState() {
    try {
      await supabase.from('game_state').update({
        game_over: true,
        winner_screen: true
      }).eq('id', 1);
      showNotification('Test winner state set', 'success');
    } catch (e: any) {
      showNotification('State update failed: ' + e.message, 'error');
    }
  }

  // --- Clear winner state function ---
  async function clearWinnerState() {
    try {
      await supabase.from('game_state').update({
        game_over: false,
        winner_screen: false
      }).eq('id', 1);
      showNotification('Winner state cleared', 'success');
    } catch (e: any) {
      showNotification('State update failed: ' + e.message, 'error');
    }
  }
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="ml-auto flex items-center gap-2">
                  <button
  onClick={async () => {
    // ensure DB is cleared of any active question / final flags and used categories before opening
    ...
    <span className="text-slate-500">No buzzes yet</span>
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              ...
              <div className="p-4 bg-[#0d1117] rounded-md border border-[#21262d]">
                <h3 className="text-xs font-bold mb-3">Cast Screen Testing</h3>
                <button onClick={testWinnerBroadcast} className="px-3 py-2 bg-purple-500 rounded-md text-xs text-white">Test Winner Broadcast</button>
                <p className="text-xs text-slate-400 mt-2">This will send a test winner to the cast screen to verify the broadcast is working.</p>
              </div>
            </div>
          )}
